...
变量的类型可以使用 Printf 函数中的 **%T** 占位符打印出来。在 Go 标准库内有一个 `unsafe <https://golang.org/pkg/unsafe/>`_ 包，包内有一个 `Sizeof <https://golang.org/pkg/unsafe/#Sizeof>`_ 方法，它可以返回传入变量所占的字节数。``unsafe`` 包如其名，它是不安全的，可能会导致一些未知的错误，慎用。
...
可以注意到上面的变量 ``a`` 和 ``b`` 都是 ``int`` 类型，并且大小为 32 位（4 字节），这是因为上面的程序是运行在 32 为操作系统上的，如果是运行在 64 位操作系统上，会出输出变量大小为 64 为（8 字节）。
